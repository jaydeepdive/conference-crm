...
export default async function InvestorDetailPage({ params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  const [{ data: inv }, { data: profiles }, { data: activity }, { data: notes }, { data: comps }, { data: compTypes }, { data: attendees }] = await Promise.all([
    ...
    supabase.from("attendee_profiles").select("*").eq("lead_type","investor").eq("lead_id", id).order("created_at",{ascending:true}),
  ]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          ...
          <LeadAttendees conferenceId={ctx.conference.id} leadType="investor" leadId={id}
