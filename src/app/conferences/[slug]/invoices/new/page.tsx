...
export default async function NewInvoicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: companies }, { data: investors }] = await Promise.all([
    supabase.from("companies").select("id,name,contact_name,email,amount_due,amount_paid").eq("conference_id", ctx.conference.id).order("name"),
    supabase.from("investors").select("id,firm_name,contact_name,email,amount_due,amount_paid").eq("conference_id", ctx.conference.id).order("firm_name"),
  ]);
