    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{rows.length} leads · target 30–50</p>
        </div>
        <Link href="/investors/new" className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand/90">+ New investor</Link>
      </div>
      <LeadTable rows={rows} profiles={profiles ?? []} basePath="/investors" />
    </div>
  );
}
